...
class MultipartWhenResource extends AbstractJsonObjectResource implements MultipartWhenResourceInitializer
{
    ...
    @Override
    public @NotNull
    MultipartWhenResourceInitializer setWhen(@NotNull Consumer<MultipartBlockPropertyResourceInitializer> consumer) {
        new RBMaybe<MultipartBlockPropertyResourceInitializer, AbstractJsonObjectResource>().provide(ResproBuilders.supplyTypedBuilder(
                        MultipartBlockPropertyResourceInitializer.class))
                .ifNotNull(when)
